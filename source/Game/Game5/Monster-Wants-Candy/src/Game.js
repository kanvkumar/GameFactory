Candy.Game = function(game) {
	...
	totalScore = 0;
	candyEnum = {
		'red': 0, 'marshmallow': 1, 'jelly': 2, 'donut': 3, 'cupcake': 4, 'pink': 5, 'lollipop': 6,
		'icecream': 7, 'teddy': 8, 'cake': 9, 'chocolate': 10, 'super': 11, 'bomb': 12
	};
	candyList = [
		'red','marshmallow','jelly','donut','cupcake','pink',
		'lollipop','icecream','teddy','cake','chocolate','super','bomb'
	];
	livesCount = 0;
	lives = null;
};
Candy.Game.prototype = {
	create: function() {
		...
		newCandyMessage.scale = { x: 0.01, y: 0.01 };
		...
		buttonContinue = this.add.button(Candy.GAME_WIDTH, Candy.GAME_HEIGHT-131-270, 'button-continue', function(){pauseButtonDisabled = false;this.managePause();}, this, 1, 0, 2);
		...
		playerBasicEating.scale = {x:2,y:2};
		...
		playerCrownEating.scale = {x:2,y:2};
		...
		playerKingEating.scale = {x:2,y:2};
		playerStatusTable = {
			'basic': { 'idle': playerBasicIdle, 'eating': playerBasicEating },
			'crown': { 'idle': playerCrownIdle, 'eating': playerCrownEating },
			'king': { 'idle': playerKingIdle, 'eating': playerKingEating }
		}
		...
		scoreText = this.add.text(120, 28, "0", { font: "40px ComicBook", fill: "#FFCC00", align: "center" });
		...
		for(var i=0; i<3; i++) {
			...
			life.scale = {x:0.01,y:0.01};
			this.add.tween(life.scale).to({x:1,y:1}, 2500, Phaser.Easing.Elastic.Out, true, 600-300*i);
		}
		...
		if(!pauseButtonDisabled) {
			...
			if(!gamePaused) {
				...
				if(newCandyMessage.visible) {
					...
					newCandyTween.to({ x: 0.01, y: 0.01 }, 500, Phaser.Easing.Elastic.Out);
				...
				this.add.tween(pausedScreenText).to( { alpha: 0 }, 300, Phaser.Easing.Linear.None, true, 0, 0, false);
				this.add.tween(buttonAudio).to( {y: -96}, 300, Phaser.Easing.Exponential.Out, true, 0, 0, false);
				this.add.tween(buttonBack).to({x: -358}, 300, Phaser.Easing.Exponential.Out, true, 0, false);
				this.manageAnimations('on');
				var continueTween = this.add.tween(buttonContinue);
				continueTween.to({ x: Candy.GAME_WIDTH }, 300, Phaser.Easing.Exponential.Out);
		...
		if(this.input.activePointer.isDown) {
			...
			if(!gamePaused) {
				if(candy.type == candyEnum.bomb) {
					...
					explosion.scale = { x: 0.01, y: 0.01 };
					candy.kill();
					this.playerAnimation('idle');
					var gameoverTween = this.add.tween(explosion.scale);
					gameoverTween.to({ x: 1, y: 1 }, 500, Phaser.Easing.Elastic.Out);
					...
					var pointsAdded = this.add.text(candy.x, candy.y, '+'+(candy.type+1),
						{ font: "48px ComicBook", fill: "#FFCC00", stroke: "#663399", strokeThickness: 10 }); // beccapurple
					pointsAdded.anchor.setTo(0.5, 0.5);
					this.add.tween(pointsAdded).to({ alpha: 0, y: destination }, 1000, Phaser.Easing.Linear.None).start();

					scoreText.setText(score);
					scoreText.x = (213+75-scoreText.width)*0.5;
					candy.body = null;
					var eatTween = this.add.tween(candy);
					eatTween.to({ x: 140, y: 800 }, 250, Phaser.Easing.Linear.None);
	...
	spawnCandy: function() {
		...
		if(randomizeCandyType == 13) { // 1% chance of having a super candy
			candyType = candyEnum.super;
		}
		else if (randomizeCandyType > 90) { // 10% chance of having a bomb
		...
		candy.body.velocity.y = -(Math.floor(Math.random()*150)+400)*2; // 2x 750-1000
	...
	update: function() {
		if(gameOver) {
			if(!runOnce) {
				...
				this.add.tween(gameOverText).to( { alpha: 1 }, 300, Phaser.Easing.Linear.None, true, 0, 0, false);
				buttonBack.bringToTop();
				buttonRestart.visible = true;
				buttonRestart.bringToTop();
				this.add.tween(buttonBack).to({x: (Candy.GAME_WIDTH-358)*0.5}, 300, Phaser.Easing.Exponential.Out, true, 0, false);
				this.add.tween(buttonRestart).to({x: (Candy.GAME_WIDTH-363)*0.5}, 300, Phaser.Easing.Exponential.Out, true, 0, false);
				pauseButtonDisabled = true;

				this.add.sprite(10, 10, 'score-bg');
				scoreText = this.add.text(120, 28, score, { font: "40px ComicBook", fill: "#FFCC00", align: "center" });
				scoreText.x = (213+75-scoreText.width)*0.5;
			}
		}
		else if(!gamePaused) {
			...
			var numberOfCandy = Math.ceil(globalTimer/20000); // [add] number of candy after every 20 seconds
			...
			if(gameNewCandy) {
				if(!runOnce2) {
					...
					this.add.tween(newCandyMessage.scale).to({ x: 1, y: 1 }, 1500, Phaser.Easing.Elastic.Out).start();
					buttonContinue.visible = true;
					buttonContinue.bringToTop();
					this.add.tween(buttonContinue).to({x: (Candy.GAME_WIDTH-358)*0.5}, 300, Phaser.Easing.Exponential.Out, true, 0, false);
					this.manageAnimations('off');
				}
			}
			else {
				if(!runOnce3) {
					...
					buttonContinue.visible = true;
					buttonContinue.bringToTop();
					this.add.tween(pausedScreenText).to( { alpha: 1 }, 300, Phaser.Easing.Linear.None, true, 0, 0, false);
					this.add.tween(buttonAudio).to( {y: 10}, 300, Phaser.Easing.Exponential.Out, true, 0, 0, false);
					this.add.tween(buttonBack).to({x: (Candy.GAME_WIDTH-358)*0.5}, 300, Phaser.Easing.Exponential.Out, true, 0, false);
					this.add.tween(buttonContinue).to({x: (Candy.GAME_WIDTH-358)*0.5}, 300, Phaser.Easing.Exponential.Out, true, 0, false);
					this.manageAnimations('off');
					pauseButtonDisabled = true;
	...
	resetCandy: function(candy) {
		...
		if(candy.type != candyEnum.super && candy.type != candyEnum.bomb) {
			...
			showTween.to({alpha:1}, 1000, Phaser.Easing.Linear.None, true, 0, 0, true);
			showTween.onComplete.addOnce(function(){killmarker.alpha=0;}, this);
			showTween.start();

			var life = lives.getFirstAlive();
			if(life) {
				this.add.tween(life.scale).to({x:0.01,y:0.01}, 500, Phaser.Easing.Back.Out, true, 0).onComplete.addOnce(function(){life.kill();}, this);
				livesCount -= 1;
			}
			if(!livesCount) {
				gameOver = true;
	...
	restartGame: function() {
		...
		restartTween.to({ x: Candy.GAME_WIDTH }, 300, Phaser.Easing.Exponential.Out);
